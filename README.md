...
so a single source tree builds on both lines.
...
5.15 also builds with `-Wdeclaration-after-statement`, which ABK turns fatal with
`-Werror`, so a declaration must not follow a statement in a function's top-level
scope. `tests/test_driver_style.py` checks that without needing a toolchain.

5.15 还会带上 `-Wdeclaration-after-statement`，而 ABK 加了 `-Werror`，所以函数
顶层作用域里的声明不能写在语句之后。`tests/test_driver_style.py` 不依赖工具链就能
检查这一点。
...
- `agent/`: Go desktop bridge that relays CTAP frames to a local virtual HID
  device (`/dev/uhid` on Linux, `\\.\ABKFidoVhid` on Windows).
- `windows/`: the `abkfidovhid` virtual HID driver, `abkvhidctl.exe`, and the
  PowerShell packaging and install scripts the Windows agent needs.
- `tests/test_driver_style.py`: toolchain-free check for the 5.15
  declaration-after-statement rule.
...
- KernelSU is optional. When `common/drivers/kernelsu/selinux/rules.c` is present
  the `/metadata` allow rules are injected; otherwise that step is skipped with a
  warning and SELinux may deny the persisted credential store.

  KernelSU 可选。存在 `rules.c` 时会注入 `/metadata` 的 allow 规则；不存在时
  跳过并告警，此时 SELinux 可能拒绝持久化的凭据存储。
...
This is the string to paste into ABK's custom external modules field:

这是填进 ABK 自定义外部模块输入框的字符串：
...
accept a branch or tag, so the default branch supports both kernel lines rather
than splitting them across branches.
...
- Local approval is mandatory: nothing that touches a credential is answered
  ...
  decision is pending the driver paces the wait with a CTAPHID keepalive
  frame (`0xbb` on the wire, one-byte `PROCESSING` status) every 500 ms,
  like hardware security keys do; Windows webauthn's HID read gives up
  after ~2 s of silence otherwise.
...
These run on any machine with Python 3.9 or newer; no kernel tree or cross
toolchain is needed:

以下检查在任何有 Python 3.9+ 的机器上都能跑，不需要内核树或交叉编译工具链：

```bash
python3 -m unittest discover -s tests -p 'test_*.py' -v
python3 -m py_compile scripts/*.py tests/*.py
bash -n setup.sh scripts/libabk.sh scripts/abk_fido_setup.sh
shellcheck --severity=warning setup.sh scripts/libabk.sh scripts/abk_fido_setup.sh
```
...
None of this compiles a kernel, so a clean run is not evidence that the kernel
build succeeds.

这些都不编译内核，所以全部通过不等于内核能编译成功。
...
- The job is skipped on forks, because signing secrets are not inherited and an
  ...
  该 job 在 fork 上会跳过，因为签名 secret 不会被继承。想从 fork 发布，需要自己
  配置上面四个 secret 并手动触发 `workflow_dispatch`。
- `ABK_EXTENSION_COMPANION_DOWNLOAD_URL` in `module.conf` points at the upstream
  release, which is the one that publishes a signed asset. Repoint it after
  publishing here.
  `module.conf` 里的 `ABK_EXTENSION_COMPANION_DOWNLOAD_URL` 指向上游 release，
  因为只有那边会发布已签名产物；等这里自己发布后再改指向。
...
- The kernel waits up to 30 s for a decision while the companion's prompt times
  out at 25 s. With the 750 ms poll interval the worst case is about 25.75 s. If
  timeouts appear on a device, lower the app's 25 s rather than raising the
  kernel's 30 s: the USB host has its own CTAPHID deadline.
  内核最多等 30 秒，应用侧提示超时是 25 秒，加上 750ms 轮询间隔最坏约 25.75 秒。
  设备上如果频繁超时，优先调小应用的 25 秒而不是调大内核的 30 秒，因为 USB 宿主
  自己也有 CTAPHID 超时。
- Unplugging the cable while a request is waiting for a fingerprint can block
  ...
  `cancel_work_sync()` without waking the sleeping thread.
  等待指纹期间拔线，可能让 gadget 拆卸阻塞到 30 秒等待结束：`abk_fido_unbind()`
  只调用了 `cancel_work_sync()`，没有唤醒正在睡眠的线程。
- The setup scripts patch source anchors and defconfig state. They do not
  compile, link, or boot a kernel.
  安装脚本只处理源码锚点和 defconfig，不编译、不链接、不启动内核。
